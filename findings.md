...
| MN-6 | HIGH | src/App.tsx:920 (handleLogin) | ROOT CAUSE of "made a note mentioning myself, no bell". Notification wiring (fetchMyNotifications + startNotificationPolling + subscribeToNotifications) lived ONLY in the session-resume path. Fresh login via handleLogin never wired it up, so the bell never fetched/subscribed for the whole session. New mentions only appeared after a full page reload. | FIXED — extracted `setupNotifications(userId)` helper, called from both resume and handleLogin |
| MN-7 | MEDIUM | Supabase config (notifications table) | Realtime NOT enabled on `notifications` table. Live test: Realtime server replied "Unable to subscribe to changes... check Realtime is enabled". So `subscribeToNotifications` (instant bell push) never delivered; mentions arrived only via 5-min poll or reload. | FIXED — `notifications` added to `supabase_realtime` publication (Dashboard > Database > Publications). Re-test as authorized recipient: INSERT event delivered instantly. |
...
## Contractor side (reported by user: Carlos Valbuena, US-15200 / US-15320 photos lost; Start Work button hidden)

| ID | Severity | File / location | Finding | Status |
|----|----------|-----------------|---------|--------|
| CB-1 | HIGH (UI) | src/index.css + JobDetail.tsx:1522 / Layout.tsx:622 | `pb-safe` (JobDetail CTA) and `safe-area-pb` (Layout nav) class names were used but NEVER defined in CSS, so they added zero clearance. The `fixed bottom-0` "Start Work Order" CTA sat flush at the screen edge and was overlapped by the device bottom toolbar / home indicator. | FIXED — defined both utilities with `calc(env(safe-area-inset-bottom) + …)` in index.css; added `z-40` to the CTA bar. |
| CB-2 | CRITICAL (data loss) | Storage bucket `customer-files` / photoStorage.ts | Carlos's photos for US-15200 (job-1780091521282) and US-15320 (job-1779994258156) are NOT in Storage. Zero files exist under ANY contractor `cj-*` prefix in `wo-photos/`; the only folders present are admin `job-*` ids. No contractor-side photo upload has ever succeeded. Photos for these two WOs are unrecoverable from the cloud (never reached Storage). Possible local-only copies may remain in Carlos's device localStorage if untouched. | OPEN — needs reproduction of contractor upload to confirm failure mode (Storage RLS path/role on `customer-files` vs network/offline). |
| CB-3 | CRITICAL (data loss) | App.tsx handleContractorJobUpdate + contractorStore (KV key `solarflow_contractor_jobs`) | Contractor jobs sync as ONE monolithic JSONB blob via `pushKeyValue` (whole-blob last-write-wins, no per-record merge unlike `solarflow_data`). Cloud copy shows Carlos's jobs as status `assigned` with 0 photos even though admin job rows are in_progress/invoiced - his session never landed (overwritten or never pushed). | OPEN — move contractor jobs to per-record sync/merge, or merge on write. Needs design + go-ahead. |
| CB-4 | MEDIUM | App.tsx ~1279 mirror dedup | Duplicate contractor-job rows for one admin job (cj-1780146343551 + cj-1780091586141 both map to job-1780091521282 / US-15200). Mirror dedup (`alreadyMirrored`) is racy / not enforced in the cloud blob. | OPEN |
