...
import {
  ...
  PauseCircle, PlayCircle, Download, Bell, CheckCircle2,
} from 'lucide-react';
...
export const ContractorDashboard: React.FC<ContractorDashboardProps> = ({
  ...
  const ListCard: React.FC<{ job: ContractorJob }> = ({ job }) => (
    <div
      ...
      // Paid orders get a green wash so a contractor can pick out what has
      // actually been settled without reading a single badge.
      className={`rounded-xl border-2 hover:shadow-md transition-all cursor-pointer p-4 ${
        job.status === 'paid'
      ...
      <div className="flex items-start justify-between gap-3">
        <div className="flex-1 min-w-0">
          ...
          {job.status === 'paid' && job.paidAt && (
            <p className="flex items-center gap-1 mt-1.5 text-xs font-semibold text-green-700">
              <CheckCircle2 className="w-3.5 h-3.5" />
              Paid {new Date(job.paidAt).toLocaleDateString('en-US', { month: 'short', day: 'numeric', year: 'numeric' })}
            </p>
          )}
        </div>
