...
/** Shared live/saved cost breakdown. Reroof materials are additional to Parts & Labor.
 * Direct parts cost is a fallback when there are no itemized non-labor costs.
 */
export function serviceCallCostBreakdown(job: Partial<Job>, expenses: { amount: number; status?: string }[] = []) {
  ...
  const reroofParts = /re-?roof/i.test(job.serviceType ?? '')
    ? (job.reroof?.parts ?? []).reduce((s, p) => s + (p.qty || 0) * (p.unitPrice ?? 0), 0) : 0;
...
export function actualServiceCallCost(job: Partial<Job>, expenses: { amount: number; status?: string }[] = []): number {
  return serviceCallCostBreakdown(job, expenses).total;
}
