...
export function toContractorJobView(job: Job, existingCj?: ContractorJob): ContractorJob {
  ...
  return {
    ...
    assignedAt: job.assignedAt ?? existingCj?.assignedAt ?? job.contractorSentAt ?? new Date().toISOString(),
    ...
    photos,
    // Phase D: read contractor fields from the SINGLE source (Job) first, falling
    // back to the legacy ContractorJob row (existingCj) for any not-yet-migrated
    // record. Job is now authoritative; existingCj is the transition safety net.
    parts: (job.contractorParts as ContractorJob['parts']) ?? existingCj?.parts ?? [],
    laborAmount: job.contractorLaborAmount ?? existingCj?.laborAmount ?? 0,
    partsAmount: job.contractorPartsAmount ?? existingCj?.partsAmount ?? 0,
    markupPercent: job.markupPercent ?? existingCj?.markupPercent ?? 0,
    ...
    contractorTotalPay: job.contractorTotalPay ?? existingCj?.contractorTotalPay ?? (job.contractorPayRate ?? 0) * ((job.contractorPayUnit ?? 'flat') === 'flat' ? 1 : (job.laborHours ?? 1)),
    paymentStatus: (job.contractorPaymentStatus as ContractorJob['paymentStatus']) ?? existingCj?.paymentStatus ?? 'pending',
    ...
    mileageCost: job.mileageCost ?? existingCj?.mileageCost,
    mileageCharge: job.mileageCharge ?? existingCj?.mileageCharge,
    // Contractor-specific fields — Job-first, existingCj fallback
    signature: job.signature ?? existingCj?.signature,
    clientSignature: job.clientSignature ?? existingCj?.clientSignature,
    signatureDate: job.signatureDate ?? existingCj?.signatureDate,
    invoiceId: job.contractorInvoiceId ?? existingCj?.invoiceId,
    invoiceStatus: (job.contractorInvoiceStatus as ContractorJob['invoiceStatus']) ?? existingCj?.invoiceStatus,
    invoiceSentAt: job.contractorInvoiceSentAt ?? existingCj?.invoiceSentAt,
    invoicePaidAt: existingCj?.invoicePaidAt,
    contractorInvoiceNumber: job.contractorInvoiceNumber ?? existingCj?.contractorInvoiceNumber,
    operationalNotes: job.serviceReport ?? existingCj?.operationalNotes,
