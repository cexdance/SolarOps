// SolarOps, workload math for the User Activity widget.
//
// change_log has no duration column, so "how long did someone work" has to be
// inferred from the timing of their writes. Events are grouped into sessions
// with an idle gap, and each session counts its first-to-last span.
//
// The 30-minute gap is measured, not guessed: across 60 days of change_log,
// 91.9% of gaps between consecutive events are under 5 minutes and the curve is
// flat from 15 to 60 minutes, so the totals barely move if you retune it.
