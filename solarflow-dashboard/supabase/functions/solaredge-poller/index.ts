...
//
// Runs 5×/day at 6am, 10am, 1pm, 3pm, 6pm ET. For each SolarEdge site:
...
//   3. Upserts a `solar:{siteId}` row in the `app_data` table
//   4. Supabase Realtime fans the change out to every connected client (<200ms)
//
...
//   Runs 5×/day at 6am, 10am, 1pm, 3pm, 6pm ET (UTC-4 EDT):
//   SELECT cron.schedule('solaredge-poll', '0 10,14,17,19,22 * * *',
...
// 300 calls/day ÷ 5 runs/day = 60 calls/run. Covers ~150 sites across 2.5 cycles/day.
const MAX_OVERVIEW_CALLS_PER_RUN = 60;
...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    const upserts = await Promise.all(ranked.map(async ({ site }) => {
      ...
      return { siteId: site.id, ok: !error, error: error?.message };
    }));
